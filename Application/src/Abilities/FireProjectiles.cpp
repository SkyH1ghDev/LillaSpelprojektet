...
void AbilityManager::Shotgun(DX::XMFLOAT2 position, DX::XMFLOAT2 direction, size_t projectileCount, float lifeSpan, float shotSpeed, float spreadAngle)
{
    ...
    for (int i = 0; i < projectileCount; ++i)
    {
        ...
        // Add projectile
        ProjectileManager::AddProjectile(ProjectileType::ScatterPellet, DX::XMFLOAT2(position.x + finalDirection.x * 15, position.y + finalDirection.y * 15), finalDirection, shotSpeed + shotSpeed + (rand() % 50 - 25), lifeSpan, 12.0f);
    }
}
...
void AbilityManager::SurroundShot(DX::XMFLOAT2 position, size_t projectileCount, float lifeSpan, float shotSpeed)
{
    ...
    for (int i = 0; i < projectileCount; ++i)
    {
        ...
        // Add projectile
        ProjectileManager::AddProjectile(ProjectileType::FireBall, DX::XMFLOAT2(position.x + finalDirection.x * 15, position.y + finalDirection.y * 15), finalDirection, shotSpeed, lifeSpan, 20.0f);
    }
}
